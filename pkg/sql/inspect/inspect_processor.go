...
var (
	...
	admissionControlEnabled = settings.RegisterBoolSetting(
		settings.ApplicationLevel,
		"sql.inspect.admission_control_enabled",
		"when enabled, INSPECT jobs use bulk low QoS; when disabled, INSPECT jobs use normal QoS",
		true,
	)
)

// getInspectQoS returns the QoS level to use for INSPECT operations based on
// the cluster setting sql.inspect.admission_control_enabled.
