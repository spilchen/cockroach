...
func InspectChecksByIndexNames(ctx context.Context, p PlanHookState, names tree.TableIndexNames) ([]*jobspb.InspectDetails_Check, error) {
	checks := []*jobspb.InspectDetails_Check{}
	...
	for _, indexName := range names {
		...
		if err != nil {
			return nil, pgerror.Wrapf(err, pgcode.InvalidName, "index not found")
		}
