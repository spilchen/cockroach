...
type progressTracker interface {
	...
	initTracker(existingProgress *jobspb.RowLevelTTLProgress, sv *settings.Values) ([]roachpb.Span, error)
...
type legacyProgressTracker struct {
	...
	// initialProgress holds the progress structure until initJobProgress is called
...
func (t *legacyProgressTracker) initTracker(
	*jobspb.RowLevelTTLProgress, *settings.Values,
) ([]roachpb.Span, error) {
	// Legacy mode doesn't use span checkpointing, so always return empty slice
	return nil, nil
}
...
func (t *checkpointProgressTracker) initTracker(
	existingProgress *jobspb.RowLevelTTLProgress, sv *settings.Values,
) ([]roachpb.Span, error) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	if existingProgress != nil && len(existingProgress.CompletedSpans) > 0 {
		...
		log.Infof(context.Background(), "TTL job restarting with %d existing completed spans", len(completedSpans))
	}
...
func (t *checkpointProgressTracker) updateProgressCache(
	...
	// Verify no overlapping spans in test builds (expensive O(n²) check)
	if buildutil.CrdbTestBuild {
		for i, span1 := range rowLevelTTL.CompletedSpans {
			for j, span2 := range rowLevelTTL.CompletedSpans {
				if i <= j {
					continue
				}
				if span1.Overlaps(span2) {
					return nil, errors.Newf("progress update contains overlapping spans: %s and %s", span1, span2)
				}
			}
		}
	}
...
// SPILLY - we should ask to stage this into two PRs. One PR should be to setup
// the progress.go interface using just the legacy progress tracker.
