...
func (t *ttlProcessor) startAsyncWork(
	...
	group.GoCtx(func(ctx context.Context) error {
		...
		for i, span := range ttlSpec.Spans {
			select {
			case <-ctx.Done():
				return errors.Wrapf(ctx.Err(), "stopping TTL processor")
			default:
				// continue work
			}
			...
			if spansProccessedSinceLastUpdate.Load() >= updateEvery &&
				...
				if err := updateFractionCompleted(); err != nil {
					log.Infof(ctx, "stopping TTL processor that writes to the bounds chan (updateFractionCompleted failure): %v", err)
					return err
				}
			}
		}
		log.Infof(ctx, "stopping TTL processor that writes to the bounds chan (clean)")
		return nil
	})
