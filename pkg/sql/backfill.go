...
func (sc *SchemaChanger) distIndexBackfill(
	...
	if err := sc.txn(ctx, func(
		...
		if err := maybeEnableDistributedMergeIndexBackfill(
			ctx, sc.execCfg.Settings, sc.execCfg.NodeInfo.NodeID.SQLInstanceID(),
			backfill.DistributedMergeConsumerLegacy, &spec,
		); err != nil {
			return err
		}
