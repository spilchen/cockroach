...
func (m *bulkMergeProcessor) processMergedData(
	...
	for {
		...
		baseKey, keyToWrite, err := m.extractKeyAndCheckDuplicate(
			key, val, usingSuffixedIter, &prevBaseKey,
		)
		if err != nil {
			return execinfrapb.BulkMergeSpec_Output{}, err
		}
...
func (m *bulkMergeProcessor) extractKeyAndCheckDuplicate(
	key storage.MVCCKey, val []byte, usingSuffixedIter bool, prevBaseKey *roachpb.Key,
) (roachpb.Key, storage.MVCCKey, error) {
	...
	if err != nil {
		return nil, storage.MVCCKey{}, err
	}

	// Check for cross-SST duplicate.
	if *prevBaseKey != nil && baseKey.Equal(*prevBaseKey) {
		return nil, storage.MVCCKey{}, kvserverbase.NewDuplicateKeyError(baseKey, val)
	}

	*prevBaseKey = baseKey.Clone()
	return baseKey, storage.MVCCKey{Key: baseKey, Timestamp: key.Timestamp}, nil
}
...
// to prevent Pebble from shadowing duplicates.
...
// with suffixing iterators to prevent Pebble from shadowing duplicates, and
// merges them. This is used when EnforceUniqueness is true.
