...
func (m *bulkMergeProcessor) ingestFinalIteration(
	...
	adder, err := m.flowCtx.Cfg.BulkAdder(ctx, m.flowCtx.Cfg.DB.KV(), writeTS, opts)
	if err != nil {
		return execinfrapb.BulkMergeSpec_Output{}, err
	}
	...
	for iter.SeekGE(storage.MVCCKey{Key: mergeSpan.Key}); ; iter.NextKey() {
		...
		// ExternalSSTReader exposes MVCCKey structs where Key is the plain user
		// key and the timestamp lives in the MVCCKey.Timestamp field. We ingest
		// at the provided write timestamp, so we intentionally ignore any
		// timestamp present on the iterator key and use the user key as-is.
		userKey := key.Key
		val, err := iter.UnsafeValue()
		if err != nil {
			return execinfrapb.BulkMergeSpec_Output{}, err
		}
		keyCopy := userKey.Clone()
		mvccVal, err := storage.DecodeMVCCValue(val)
		if err != nil {
			return execinfrapb.BulkMergeSpec_Output{}, err
		}
		mvccVal.Value.ClearChecksum()
		mvccVal.Value.InitChecksum(keyCopy)
		encVal, err := storage.EncodeMVCCValue(mvccVal)
		if err != nil {
			return execinfrapb.BulkMergeSpec_Output{}, err
		}
		if err := adder.Add(ctx, keyCopy, encVal); err != nil {
			return execinfrapb.BulkMergeSpec_Output{}, err
		}
	}
	if !adder.IsEmpty() {
