...
// TelemetryStatusReporter is the interface to that we use to find the last ping
...
// GetGracePeriodEnd returns the timestamp marking the end of the grace period.
...
func (e *Enforcer) GetGracePeriodEnd() (time.Time, bool) {
...
// GetAllowedTelemetryDelay returns a timestamp of when telemetry
// data needs to be received before we start to throttle. If the license doesn't
// require telemetry, then false is returned for the bool.
func (e *Enforcer) GetAllowedTelemetryDelay() (time.Time, bool) {
...
func (e *Enforcer) MaybeFailIfThrottled(ctx context.Context, txnsOpened int64) (err error) {
	...
	if gracePeriodEnd, ok := e.GetGracePeriodEnd(); ok && now.After(gracePeriodEnd) {
		if e.GetHasLicense() {
	...
	if ts, ok := e.GetAllowedTelemetryDelay(); ok && now.After(ts) {
		err = errors.WithHintf(pgerror.Newf(pgcode.CCLValidLicenseRequired,
...
func (e *Enforcer) maybeLogActiveOverrides(ctx context.Context) {
	...
	curGracePeriod := e.getGracePeriodLength(maxGracePeriod)
