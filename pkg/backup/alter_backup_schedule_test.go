...
`)
		...
		// correct schedule. RUN SCHEDULE doesn't support this option, so we always
		...
		if useAlterExecute {
			...
			execSql = fmt.Sprintf(`RUN SCHEDULE %d;`, scheduleID)
		}
		...
		if !useAlterExecute {
			// RUN SCHEDULE does not return next_run for paused schedules. So,
			// we query it ourselves.
			require.Len(t, rows, 0)
			rows = th.sqlDB.QueryStr(t, fmt.Sprintf(`SELECT * FROM [SHOW SCHEDULES] WHERE id = %d`, scheduleID))
		}
