...
// registerTTLRestart will verify TTL continues to function across all ranges
// when a node is restarted in the middle of a TTL job.
...
// runTTLRestart will run the test. It ensures that TTL deletions happen across
// all ranges if a node was restarted in the middle of a TTL job.
func runTTLRestart(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	m.Go(func(ctx context.Context) error {
		...
		for _, stmt := range ddl {
			if _, err := db.ExecContext(ctx, stmt); err != nil {
				return errors.Wrapf(err, "error with ddl: %s", stmt)
			}
		}
		...
		ttlJobSettingSQL := fmt.Sprintf(`ALTER TABLE ttldb.tab1
           ...
           ttl_select_batch_size=50,
           ttl_delete_batch_size=50,
           ttl_select_rate_limit=50,
           ttl_job_cron='%s')`,
			ttlCronExpression)
		...
		waitForTTLJob := func() error {
			var err error
			jobInfo, err = findRunningJob(ctx, t, c, db, nil, false /* expectJobRestart */)
			return err
		}
		...
		t.Status("stop a non-coordinator node")
		targetNode := 2
		if targetNode == jobInfo.CoordinatorID {
			targetNode = 3
		}
		m.ExpectDeath()
		t.L().Printf("stopping node %d", targetNode)
		c.Stop(ctx, t.L(), option.DefaultStopOpts(), c.Nodes(targetNode))

		t.Status("ensure TTL job restarts")
		waitForTTLRestart := func() error {
			jobInfo, err = findRunningJob(ctx, t, c, db, &jobInfo, true /* expectJobRestart */)
			if err != nil {
				return err
			}
			return nil
		}
		testutils.SucceedsWithin(t, waitForTTLRestart, 4*time.Minute)

		t.Status("restart the down node")
		t.L().Printf("starting node %d", targetNode)
		c.Start(ctx, t.L(), startOpts, settings, c.Node(targetNode))
		m.ResetDeaths()

		t.Status("ensure TTL job restarts again")
		testutils.SucceedsWithin(t, waitForTTLRestart, 4*time.Minute)
...
func gatherLeaseDistribution(
	...
	if err != nil {
		return leases, errors.Wrapf(err, "error querying the ranges after relocation")
	}
	defer ranges.Close()
	for ranges.Next() {
...
// findRunningJob is a helper that checks if the TTL job is running and returns
// state on it. If expectJobRestart is true, we validate that the job has
// restarted. If expectJobRunning is true, an error is returned if the job isn't
// running.
func findRunningJob(
	ctx context.Context,
	t test.Test,
	...
	for i := range results {
		...
		if err != nil {
			return newJobInfo, errors.Wrapf(err, "error converting command output to int: %s", results[i].Stdout)
		}
